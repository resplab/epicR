# US_Calibrate Demographics

In this vignette, we will walk through the process of optimizing
parameters for two equations: the longevity equation `l_inc_betas` and
the incidence equation `ln_h_bgd_betas`. Specifically, we aim to
optimize these parameters to minimize the Root Mean Squared Error (RMSE)
between the projected population sizes and actual U.S. population data
for certain age groups.

We perform optimization using the optim function in R, and the goal is
to fine-tune the model’s parameters so that the RMSE is minimized. We
...
significantly high, contributing excessively to the overall error.
...
The `calculate_rmse_optim` function calculates the RMSE for each
iteration of parameter optimization. Within this function:

1.  We assign the first three values of the parameter vector
    (params\[1:3\]) to l_inc_betas, which represents parameters for the
    longevity equation.
2.  The next four values (params\[4:7\]) are assigned to the
    ln_h_bgd_betas vector, which is part of the incidence equation.
    3)The model is then run using the updated parameters, and we extract
    the projected population sizes (EPIC_popsize) and calculate the RMSE
    between the projected and observed U.S. population data for the
...
We now define our initial guess for the parameters and set the bounds
...
After running the optimization process, we obtain the optimal values for
the parameters. These values minimize the RMSE for the specified age
groups (40-59 and 60-79) and exclude the 80+ age group.
...
These are the optimized parameters that yield the lowest RMSE, and they
reflect the best possible values for the model in terms of population
size predictions for the selected age groups.

### Visualization

In this section, we visualize and compare the population trends over
