...
#DEBUG all covariates zero - except for intercept
     ...
     fev1_betas_by_sex <- cbind(male = c(intercept = -0.1543 - 0.00762, baseline_age = 0.002344, baseline_weight_kg = 0.000126, height =0, height_sq = 0, current_smoker = -0.03074, age_height_sq = 0, followup_time =0*-0.00146),
                            female = c(intercept = -0.1543 , baseline_age = 0.002344, baseline_weight_kg =0.000126, height =0, height_sq =0, current_smoker =  -0.03074, age_height_sq = 0, followup_time =0*-0.00146)) #debug
     


   dfev1_re_rho <- 0.09956332 # From G Matrix: 0.00087/(sqrt(0.1006)*sqrt(0.000759))
   dfev1_sigmas <- t(as.matrix(c(sigma1 = sqrt(0.1006), sigma2 = 0*sqrt(0.000759)))) #debug
  ...
  df <-data.frame(fev1_rate_avg, fev1_low, fev1_up,  fev1_avg)
...
ggplot(rate, aes(x = LHS_expected_FEV1   , y = epicR_predicted_FEV1)) +
  geom_point (shape = ".", size = 1, aes(colour = factor(WithinPI))) + labs(title = "Lung Function", caption = "Continuous smoker males over 40 years with COPD.", y = "EPIC-Simulated FEV1", x = "LHS Model FEV1") + geom_ribbon(aes(ymin=(LHS_expected_FEV1_low), ymax= (LHS_expected_FEV1_up)), linetype=2, alpha=0.3) + xlim(3,4) + ylim(3,4)
