...
struct output_ex
{
  ...
  int n_case_detection_by_ctime_sex[1000][2];
...
List Cget_output_ex()
{
  ...
  out["n_COPD_by_ctime_sex"]=AS_MATRIX_INT_SIZE(output_ex.n_COPD_by_ctime_sex,input.global_parameters.time_horizon),
    ...
    out["n_case_detection_by_ctime_sex"]=AS_MATRIX_INT_SIZE(output_ex.n_case_detection_by_ctime_sex,input.global_parameters.time_horizon),
...
void update_output_ex(agent *ag)
{
  ...
  //if(time>=(*ag).time_at_creation)
  {
    ...
    if((*ag).smoking_status==1)
    {
      ...
      output_ex.n_case_detection_by_ctime_sex[time][(*ag).sex]+=((*ag).case_detection>0)*1;;
