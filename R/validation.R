...
validate_populationUS <- function() {
  ...
  USSimulation <- read_csv("data-raw/USCensus.csv")
...
validate_smokingUS <- function(remove_COPD = 1, intercept_k = NULL) {
  message("Welcome to EPIC validator! Today we will see if the model make good smoking predictions in the US")
  petoc()
  library(tidyverse)
  ...
  message("There are two validation targets: 1) the prevalence of current smokers, former smokers, and non-smokers in 2018; 2) the prevalence of current smokers in 2023, and 3) the projected decline in smoking rate.\n")
  message("Starting validation target 1 and 2.\n")
  petoc()

  # CanSim.105.0501<-read.csv(paste(data_path,'/CanSim.105.0501.csv',sep=''),header=T) Included in the package as internal data

  USSmoking2018 <- read_csv("data-raw/USSmoking2018.csv")
  ...
  barplot(tab1, names.arg = c("40-64", "65-74", "75+"), ylim = c(0, 0.4), xlab = "Age group", ylab = "Prevalence of smoking",
          col = c("grey"))
  title(cex.main = 0.5, "Prevalence of current smoker by age group (observed)")
...
#' Returns results of validation tests for exacerbation rates for the US population
#' @param base_agents Number of agents in the simulation. Default is 1e4.
#' @param input EPIC inputs
#' @return validation test results
#' @export
