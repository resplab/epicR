...
For advanced users who need fine-grained control (e.g., running multiple simulations in one session), you can manage sessions manually.
...
`init_session()` is in the **core.R** file, and calls 3 C functions: `deallocate_resources()`,
`allocate_resources()`, and `init_session_internal()`. You don't really need to know how these work, but basically they allocate/deallocate memory.
...
(Already shown above with `init_session()`)
...
The first function is the `get_output()` which shows some of the overall results:

```{r, eval=FALSE, echo=TRUE}
results <- get_output()
```

**Basic Output from `get_output()`:**
...
The second function `get_output_ex()` returns a very large list of results:

```{r, eval=FALSE, echo=TRUE}
resultsExra <- get_output_ex()
```

**Extended Output from `get_output_ex()`:**
...
`get_all_events_matrix()` function returns the events matrix, with every event for every patient. Note that you might need a large amount of memory available, if you want to collect event history for a large number of patients.
...
As shown in the code snippet above, you can inspect the event matrix `get_all_events_matrix()` by converting it into a data frame using `as.data.frame(get_all_events_matrix())`. This data frame consists of 33 columns, including:
