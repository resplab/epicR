...
The easiest way to run a simulation is using the `simulate()` function, which handles all session management automatically and provides progress information (configuration summary, elapsed time, and status messages):
...
1. Reduce `n_base_agents`
2. Use `record_mode = 0` (aggregate only)
